...
def get_history_events(since=None, limit=100):
    try:
        limit = int(limit)
    ...
    if redis_client:
        try:
            raw_events = redis_client.lrange(LOCK_HISTORY_KEY, -limit, -1)
    ...
    if not events:
        for lock in RecordingLock.objects.exclude(history=[]):
            events.extend(lock.history or [])

    if since:
        events = [event for event in events if event.get("timestamp") and event["timestamp"] >= since]
