...
from .recording_locks import get_active_lock, get_history_events
from .models import (
    Stream,
    Channel,
    ChannelGroup,
...
class RecordingLockHistoryAPIView(APIView):
    permission_classes = [Authenticated]

    def get(self, request):
        since = request.query_params.get("since")
        limit = request.query_params.get("limit", 100)
        events = get_history_events(since=since, limit=limit)
        return Response(
            {
                "events": events,
            },
